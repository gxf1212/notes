...
\begin{tcolorbox}
	...
	\end{equation}
\end{tcolorbox}
is called the \textit{mass metric matrix} (of $\alpha$ and $\beta$) and is the function of $q$ (so is $U(\boldsymbol{q})$). Then the Lagrangian is expressed as a function of $\boldsymbol{q}$ and $\dot{\boldsymbol{q}}$. Adopting the Euler-Lagrangian equation, considering $q_{\gamma}$ as $q_\alpha$:
\begin{equation}
