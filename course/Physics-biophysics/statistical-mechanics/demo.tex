...
\begin{tcolorbox}
	...
	\end{equation}
	is the function of $q$ (so is $U(\boldsymbol{q})$). $\alpha$ and $\beta$ as indices, these elements forms a matrix $G$, the \textit{mass metric matrix}. 
\end{tcolorbox}
Then the Lagrangian is expressed as a function of $\boldsymbol{q}$ and $\dot{\boldsymbol{q}}$. Adopting the Euler-Lagrangian equation, considering $q_{\gamma}$ as $q_\alpha$:
\begin{equation}
...
\end{equation*}
holds, where $b(x_0)$ is the $y$-intercept. Thus, forall $x$
\begin{equation}
	f(x)=f^\prime(x)x+b(x)
\end{equation}
